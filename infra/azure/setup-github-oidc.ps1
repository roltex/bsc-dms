...
Write-Host "  AZURE_WEBAPP_NAME     = app-efes-dms-prod"
Write-Host "  AZURE_RESOURCE_GROUP  = $ResourceGroup"
Write-Host "  AZURE_WEBAPP_HOST     = app-efes-dms-prod.azurewebsites.net"
Write-Host ""
...
if (Get-Command gh -ErrorAction SilentlyContinue) {
    ...
    if ($confirm -eq 'y') {
        ...
        gh variable set AZURE_WEBAPP_NAME --body "app-efes-dms-prod" -R "$GitHubOrg/$GitHubRepo"
        gh variable set AZURE_RESOURCE_GROUP --body $ResourceGroup -R "$GitHubOrg/$GitHubRepo"
        gh variable set AZURE_WEBAPP_HOST --body "app-efes-dms-prod.azurewebsites.net" -R "$GitHubOrg/$GitHubRepo"
